# Ref: Visual Transformers: https://github.com/tahmid0007/VisualTransformers/blob/main/ResViT.py
...
class ImageModelViT(nn.Module):
    def __init__(self, opt, config, out_features, device="cpu" , model="resnet18",
        dim = 128, num_tokens = 8, mlp_dim = 256, heads = 8, depth = 6, emb_dropout = 0.1, dropout= 0.1):
        super(ImageModelViT, self).__init__()
        self.device = device
        self.opt = opt 
        self.config = config
        ...
        self.nn1 = nn.Linear(dim, out_features)  # if finetuning, just use a linear layer without further hidden layers (paper)
